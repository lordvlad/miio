...
module.exports = class extends Vacuum.with(
	...
	/**
 	 * Resumes previous zoomed cleanup, if paused for whatever reason
 	 */
 	resumeZoneCleanup() {
   ...
   setLandmark(landmark, position) {
	   this._landmarks[landmark] = position
   }
   ...
   activateLandmarkClean(landmark, cleans = 2) {
	 // Zones example: [[26234,26042,26634,26642,1],[26232,25304,27282,25804,2],[26246,24189,27296,25139,2]]
	 if (!(landmark in this._landmarks)) {
		   return Promise.reject(new Error(`Unknown landmark ${landmark}`))
	 }
	 const zone = [...this._landmarks[landmark], cleans]
	 return this.activateZoneClean([zone])
   }
   ...
   goToLandmark(landmark) {
	   ...
	   let target = this._landmarks[landmark]

	   // if target is actually a zone, get the center of the zone
	   if (target.length === 4) {
		   target = [(target[0] + target[2] / 2), (target[1] + target[3] / 2)]
	   }
   ...
   /**
	 ...
	 * Takes in a single target: [25000,25000]
	 * Maps are always 51200 x 51200. The charger/starting location is always the center; so 25600, 25600
	 */
	goToTarget(target) {
		return this.call('app_goto_target', target, {
			refresh: [ 'state' ],
			refreshDelay: 1000
		})
			.then(checkResult);
